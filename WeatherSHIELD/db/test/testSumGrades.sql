...
WITH hm AS (
    SELECT
        ...
        --SUM([HINDCAST].[FCT_GradeHistoric](1, WHICH_MONTH))/(SUM([HINDCAST].[FCT_GradeHistoric](1, WHICH_MONTH)) - SUM([HINDCAST].[FCT_GradeHistoric](@MinRatioScore, WHICH_MONTH))) AS GRADE3,
...
(SELECT
    ...
    (SELECT SUM([GRADE3])
        FROM hm
        WHERE [GRADE3]>=m.[GRADE3]
        AND [GRADE3]>=0
        AND [Generated]=m.[Generated]) AS SUM_GRADE,
    (SELECT COUNT([GRADE3])
        FROM hm
        WHERE [GRADE3]>=m.[GRADE3]
        AND [GRADE3]>=0
        AND [Generated]=m.[Generated]) AS PICKED_YEARS
FROM hm m
) s
WHERE [SUM_GRADE] <= @TargetSum
ORDER BY [Generated], [GRADE3] DESC
--SELECT *
--FROM hm
--ORDER BY hm.[Generated], [GRADE] DESC