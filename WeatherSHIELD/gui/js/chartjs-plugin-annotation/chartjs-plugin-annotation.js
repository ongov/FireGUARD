...
module.exports = function(Chart) {
	...
	var BoxAnnotation = Chart.Annotation.Element.extend({
		...
		draw: function() {
			...
			// Draw
			var b = this.chartInstance.boxes[2];
			var tickWidth = (b.right - b.left) / b._ticks.length;
			var width = view.right - view.left + tickWidth / 2,
				height = view.bottom - view.top;
			if (view.left > b.left)
            {
                //HACK: fix area starting at center of tick
                ctx.fillRect(view.left + tickWidth / 2, view.top, width, height);
                ctx.strokeRect(view.left+ tickWidth / 2, view.top, width, height);
            }
            else
            {
                //HACK: fix area ending at center of tick
...
module.exports = function(Chart) {
	...
	var LineAnnotation = Chart.Annotation.Element.extend({
		...
		draw: function() {
			...
			// Draw
            // HACK: fix drawing in the center of bar
			var b = this.chartInstance.boxes[2];
			var tickWidth = (b.right - b.left) / b._ticks.length;
			ctx.beginPath();
			ctx.moveTo(view.x1 + tickWidth / 2, view.y1);
			ctx.lineTo(view.x2 + tickWidth / 2, view.y2);
			ctx.stroke();
			...
			if (view.labelEnabled && view.labelContent) {
				...
				ctx.fillText(
					view.labelContent,
					view.labelX + (view.labelWidth / 2) + tickWidth / 2,
